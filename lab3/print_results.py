...
def print_results(results, title):
	...
	for name, values in results.items():
		...
		total_rate = ((r_cnt + w_cnt) - (r_hit + w_hit)) / (r_cnt + w_cnt) * 100 if (r_cnt + w_cnt) > 0 else 0 
...
if __name__ == '__main__':
	...
	if result_path.is_dir():
		...
		for result_file in result_path.glob('**/*.out'):
			...
			max_inst = max(max_inst, results['physical index physical tag'][0] + results['physical index physical tag'][2])
